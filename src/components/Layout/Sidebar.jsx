...
import { LayoutDashboard, FileText, Search, Github, Cloud, BookOpen, Settings, LogOut, ShieldCheck, Users } from 'lucide-react';
import { useAuth } from '../../contexts/AuthContext.jsx';
import { useTenant } from '../../contexts/TenantContext.jsx';
import { ClearanceBadge } from '../ClearanceBadge.jsx';
import { CLEARANCE_LEVELS } from '../../utils/clearance.js';

const NavItem = ({ to, icon: Icon, label, restricted }) => (
  <NavLink
    to={to}
    className={({ isActive }) =>
  ...
  return (
    <aside className="w-60 flex-shrink-0 bg-slate-900 border-r border-slate-800 flex flex-col h-screen">
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
        ...
        <NavItem to={`${base}/documents`} icon={FileText} label="Document Hub" />
        <NavItem to={`${base}/search`} icon={Search} label="AI Search" />
        <NavItem to={`${base}/onboarding`} icon={BookOpen} label="Onboarding Guide" />

        <p className="px-3 pt-4 pb-2 text-xs font-semibold text-slate-500 uppercase tracking-wider">Integrations</p>
        <NavItem to={`${base}/github`} icon={Github} label="GitHub" restricted={userLevel < 3} />
        <NavItem to={`${base}/azure`} icon={Cloud} label="Azure DevOps" restricted={userLevel < 3} />

        {user?.isAdmin && (
          <>
            <p className="px-3 pt-4 pb-2 text-xs font-semibold text-slate-500 uppercase tracking-wider">Admin</p>
            <NavItem to={`${base}/admin`} icon={Users} label="Admin Panel" />
          </>
        )}
      </nav>
